...
def solve99in_oneway(list99):
    while(tru1==1):
        exanum=show_exa_num(list99)
        if(exanum==0):
            return list99
        else:
            listpro=wherex(exanum,list99)
            for y in range(9):
                for x in range(9):
                    if (show_times_in_col(listpro,x,0)==1 or show_times_in_row(listpro,y,0)==1 or show_times_in_square(listpro,int(y/3)+1,int(x/3)+1,0)==1) and listpro[y][x]==0:#列
                        list99[y][x]=exanum

            
# 根据已知最小可能的数字及其位置来递归讨论并更改多种可能的情况,mode为0只求一解，mode为1求所有解
# 保证传入的数独已按照一种解法的解完！！！savelist非常重要，保证尝试失败后能够原样恢复尝试前的数独
def solve99in_mulway(listtemp,mode):
    savelist=copy.deepcopy(listtemp)
    newlisttemp=solve99in_oneway(listtemp)#将尝试的数独进行单一解（对递归后的尝试有实际作用）
    ...
    for each_post in selnum_post:
        newlisttemp2=changexyz(each_post[0],each_post[1],selnum,newlisttemp)
...
if __name__=='__main__':
    ...
    while(tru1==1):
        ...
        if (tempkey=="1"):
            ...
            mode=int(choosemode)
            list99=solve99in_oneway(list99)
            solve99in_mulway(list99,mode)#传入的必须是按照唯一解法尽可能解到低的数独，所以上一步很重要
